...
import { saveAlertPreferences, type AlertPrefState } from './actions'
...
export default function AlertSettings({ companyId, prefs }: Props) {
  ...
  return (
    <div style={{ background: '#fff', border: '0.5px solid #dde0cc', borderRadius: 12 }}>
      ...
      >
        <div>
          <span style={{ fontSize: 13, fontWeight: 500, color: '#1e2118' }}>Alert Settings</span>
          {!expanded && (
            <span style={{ fontSize: 11, color: '#9aa090', marginLeft: 8 }}>
              {emailOn ? 'Email on' : ''}{emailOn && waOn ? ' · ' : ''}{waOn ? 'WhatsApp on' : ''}
              {!emailOn && !waOn ? 'Alerts off' : ''}
            </span>
          )}
        </div>
        <span style={{
        ...
        <div style={{ borderTop: '0.5px solid #eaecda' }}>
          <form action={formAction}>
            ...
            <div style={{ padding: '14px 16px' }}>
              {/* Alert days */}
              <div style={{ fontSize: 11, fontWeight: 500, color: '#6b7061', marginBottom: 8, letterSpacing: '0.03em' }}>
                SEND ALERTS
              </div>
